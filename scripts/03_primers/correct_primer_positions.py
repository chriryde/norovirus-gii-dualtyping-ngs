...
def update_positions(primer_df, record_dict):
    ...
    try:
        ...
        for _, row in primer_df.iterrows():
            ...
            primer_match = find_iupac_matches(primer, reference, max_l_dist)
            rev_primer_match = find_iupac_matches(revprimer, reference, max_l_dist)
            if len(primer_match) == 1:
                ...
                best_forward = find_best_iupac_match(primer, reference)
                best_reverse = find_best_iupac_match(revprimer, reference)

                print("Best forward match:")
                print(best_forward)

                print("Best reverse match:")
                print(best_reverse)
